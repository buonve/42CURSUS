...
int main()
{
	...
	try
	{
		std::cout << "Found element at position " << std::distance(int_vec.begin(), easyfind(int_vec, 10))  << std::endl;
	}
	...
	std::cout << "(La posizione non può essere trovata nelle liste in quanto la memoria non è occupata consecutivamente)" << std::endl;
	std::cout << "Finding element 3..." << std::endl;
	try
	{
		std::cout << "Found element at position " << std::distance(int_list.begin(), easyfind(int_list, 1)) << std::endl;
